...
import FertigationApi from "../../apis/advisory.api";
...
const WishlistCards = ({  setMenu }) => {
  ...
  const fertigationApi = new FertigationApi();
  const [isLoading, setIsLoading] = useState(true);

  const getFertigationData = async () => {
    try {
      const getFertigationAdvisory = await fertigationApi.getFertigationData({

      });
      if (
        getFertigationAdvisory &&
        getFertigationAdvisory.data.code === 200 &&
        getFertigationAdvisory.data.status === "success"
      ) {
        setFertigationData(getFertigationAdvisory.data.data);
        // console.log("getCropAdvisory", getFertigationAdvisory.data.data)
      }
    } catch (error) {
      console.error(error);
  ...
  const getFertigationStatus = async () =>{
    try{
      const fertigationStatus = await fertigationApi.getFertigationReadStatus();
      if(fertigationStatus || fertigationStatus.data.data){
        // console.log("profileData.data.data",fertigationStatus.data.data)
    ...
    const getData = async () =>{
      try{
        const profileData = await fertigationApi.getProfileData();
    ...
    <div className="wishlist--container">
      ...
      <Flex width={'100%'} flexDirection={'column'}>
        <Flex width={'100%'}>
         ...
         <Text bgColor={'#19884A'} width={'100%'} color={'white'} textTransform={'uppercase'} fontSize={'16px'} p={'10px 20px'}> Fertigation </Text>
         </Flex>
          <div >
          {isLoading ? (
