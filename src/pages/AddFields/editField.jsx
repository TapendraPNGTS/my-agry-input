...
import { Flex, Skeleton, Image, Input, Box, Text, Heading, Button, VStack, Card, InputGroup, InputRightElement, Select, Spinner } from "@chakra-ui/react";
...
// import FieldApi from "../../../apis/auth/auth.api";
...
const WishlistCards = ({ setMenu }) => {
  ...
  const getFieldHistory = async () => {
    try {
      const response = await serviceApi.getFieldData({ fieldId: params.id });
      if (response && response.data && response.data.data) {
        ...
        setLandDetail(field.landDetail);
        setLandMark(field.landMark);
        setStateId(field.StateID);
        setCityId(field.DistrictID);
        setLocationName(field?.location?.locationName);
        setLatitude(field?.location?.latitude);
        setLongitude(field?.location?.longitude);
        setSearchQuery(field?.location?.locationName);
        setCurrentLocation({
          city: field?.location?.locationName,
          latitude: field?.location?.latitude,
          longitude: field?.location?.longitude,
        })
      } else {
        console.error("Invalid response or data format from API");
        // Handle the case when the response or data format is invalid
      }
    } catch (error) {
  ...
  const handleSubmit = useCallback(async (event) => {
    ...
    if (valid) {
    try {
      const getfieldResponse = await serviceApi.editFields({
        fieldName: fieldName,
        cropName: cropName,
  ...
  return (
    ...
    <Box>
      ...
      <Flex justifyContent={'center'} gap={10} mt={10} mb={10}>
        {loading === true ? <Spinner color='green' /> :
        <Button _hover={'none'}
          _active={'none'}
          color={'white'}
          bgColor={'#006C1E'}
...
export default EditFieldsMapBox;
